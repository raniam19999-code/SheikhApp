...
export function showToast(message, type = "success", duration = 4000, onClick = null) {
  const container = document.getElementById("notification-container");
  const toast = document.createElement("div");
  const colors = { success: "from-[#1B4332] to-[#2D6A4F]", error: "from-red-800 to-red-950", info: "from-[#1B4332] to-slate-800", warning: "from-amber-500 to-amber-600" };
...
export function createNotification(title, message, type = "info", icon = "bell", actionText = null, actionFn = null) {
  const notification = { id: Date.now(), title, message, type, icon, actionText, actionFn, createdAt: new Date(), read: false };
...
export function getLocationGPS() {
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      document.getElementById("order-coords").value = `${pos.coords.latitude},${pos.coords.longitude}`;
      document.getElementById("order-address").value += ` [تم تحديد الموقع عبر GPS]`;
...
export function navigateBack() {
  if (window.activeSubCategoryName !== null) {
    ...
    const parent = window.categories.find(c => c.id === window.currentParentId);
    document.getElementById("current-category-title").innerHTML = `<i data-lucide="folder" class="w-5 h-5 text-emerald-500"></i> فرع: ${parent ? parent.name : ""}`;
    window.renderCategories();
    window.renderSubCategoriesAsGrid(window.currentParentId);
  } else {
    const current = window.categories.find(c => c.id === window.currentParentId);
    window.currentParentId = current ? current.parentId || null : null;
    if (window.currentParentId === null) {
      document.getElementById("current-category-title").innerHTML = `<i data-lucide="grid" class="w-5 h-5 text-[#1B4332]"></i> الأقسام الرئيسية`;
      window.renderCategories();
      window.renderProducts(window.products);
    } else {
      const parent = window.categories.find(c => c.id === window.currentParentId);
      document.getElementById("current-category-title").innerHTML = `<i data-lucide="folder" class="w-5 h-5 text-emerald-500"></i> فرع: ${parent.name}`;
      window.renderCategories();
      window.renderSubCategoriesAsGrid(window.currentParentId);
    }
  }
  window.updateBulkAddButton();
  lucide.createIcons();
}

export function togglePaymentUI() {
  const method = document.querySelector('input[name="payment-method"]:checked').value;
  document.getElementById("visa-form").classList.toggle("hidden", method !== "visa");
}
